			...
			public void run(){
					try {
						if (message.text.replace(" ", "%20").length() < 100)
							...
							playSplitAudio();
						CCComponents.NETWORK.sendToServer(new AudioResponsePacket(message.text, message.lang, message.x, message.y, message.z, Minecraft.getMinecraft().theWorld, message.side));
					} catch (Exception e) {
						e.printStackTrace();
					} finally {
						this.disable(true);
