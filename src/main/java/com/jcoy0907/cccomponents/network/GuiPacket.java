	...
	public static class GuiPacketHandler implements IMessageHandler<GuiPacket, IMessage> {
		...
		public IMessage onMessage(GuiPacket message, MessageContext ctx) {
			if (message.close)
				...
				Minecraft.getMinecraft().thePlayer.openGui(CCComponents.instance, Reference.GUIs.HELMET.ordinal(), Minecraft.getMinecraft().thePlayer.worldObj, (int) Minecraft.getMinecraft().thePlayer.posX, (int) Minecraft.getMinecraft().thePlayer.posY, (int) Minecraft.getMinecraft().thePlayer.posZ);
			return null;
		}
	}
}
