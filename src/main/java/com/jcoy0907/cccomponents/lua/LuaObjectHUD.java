...
import com.jcoy0907.cccomponents.network.CommandPacket;
import com.jcoy0907.cccomponents.network.GuiPacket;
import com.jcoy0907.cccomponents.smarthelmet.*;
...
//		try {
		switch (method) {
			...
			case 8:
				CCComponents.NETWORK.sendTo(new CommandPacket(stackToArray(), uuid, false, isGui), (EntityPlayerMP) player);
				MessageCommand.messageStacks.get(player.getDisplayName()).clear();
				renderStack.clear();
				renderStack.add(messageCommand);
				break;
			case 9:
				CCComponents.NETWORK.sendTo(new CommandPacket(new HelmetCommand[0], uuid, false, isGui), (EntityPlayerMP) player);
				MessageCommand.messageStacks.get(player.getDisplayName()).clear();
				renderStack.clear();
				renderStack.add(messageCommand);
				break;
			case 10:
				CCComponents.NETWORK.sendTo(new CommandPacket(stackToArray(), uuid, true, isGui), (EntityPlayerMP) player);
				MessageCommand.messageStacks.get(player.getDisplayName()).clear();
				renderStack.clear();
				renderStack.add(messageCommand);
				break;
			case 11:
				if (isGui)
					CCComponents.NETWORK.sendTo(new GuiPacket(false), (EntityPlayerMP) this.player);
				else
					return new Object[]{new LuaObjectHUD(this.player.getDisplayName(), this.uuid, true)};
				break;
			case 12:
				CCComponents.NETWORK.sendTo(new GuiPacket(true), (EntityPlayerMP) this.player);
				break;
			case 13:
