package com.jcoy0907.cccomponents.villagers;
...
public class TradeHandler implements VillagerRegistry.IVillageTradeHandler {
		...
		do {
			...
			switch (trade) {
				...
				case 6://64 emeralds = portable computer (an IPhone basically)
					...
					iPhone.setStackDisplayName(StatCollector.translateToLocal("item.cccomponents:iphone.name"));
	...
	public static ItemStack getBookFromInt(int type) {
		...
		switch (type) {
			case 0:
				stack.stackTagCompound = Util.writeToBookNBT("cccomponents.lore.1.title", Colors.MAGIC+"dan200", getTextFromInt(type));
				break;
			case 1:
				stack.stackTagCompound = Util.writeToBookNBT("cccomponents.lore.2.title", Colors.MAGIC+"dan200", getTextFromInt(type));
				break;
			case 2:
				stack.stackTagCompound = Util.writeToBookNBT("cccomponents.lore.3.title", Colors.MAGIC+"dan200", getTextFromInt(type));
				break;
		}
		return stack;
	}
	...
	private static List<String> getTextFromInt(int type) {
		...
		switch (type) {
			case 0:
				list.add("cccomponents.lore.1.header");
				list.add("cccomponents.lore.1.pg1");
				list.add("cccomponents.lore.1.pg2");
				list.add("cccomponents.lore.1.pg3");
				list.add("cccomponents.lore.1.pg4");
				break;
			case 1:
				list.add("cccomponents.lore.2.header");
				list.add("cccomponents.lore.2.pg1");
				list.add("cccomponents.lore.2.pg2");
				list.add("cccomponents.lore.2.pg3");
				list.add("cccomponents.lore.2.pg4");
				list.add("cccomponents.lore.2.pg5");
				list.add("cccomponents.lore.2.pg6");
				break;
			case 2:
				list.add("cccomponents.lore.3.header");
				list.add("cccomponents.lore.3.pg1");
				list.add("cccomponents.lore.3.pg2");
				list.add("cccomponents.lore.3.pg3");
				break;
		}
		return list;
	}
