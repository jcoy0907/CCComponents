...
public class Transformer implements IClassTransformer, Opcodes {
	...
	public byte[] transform(String className, String newClassName, byte[] byteCode) {
		if (className.equals("dan200.computercraft.shared.pocket.items.ItemPocketComputer")) {
			CCComponents.LOGGER.info("Adding pocket computer peripherals");
			...
			if (m.name.equals("createServerComputer")) {
				...
				while (nodes.hasNext()) {
					...
					if (node.getOpcode() == INVOKEVIRTUAL) {
						...
						instructions.add(new MethodInsnNode(INVOKESTATIC, "com/jcoy0907/cccomponents/hooks/ComputerCraftHooks", "onPocketComputerCreate", "(Ldan200/computercraft/shared/computer/core/ServerComputer;Lnet/minecraft/item/ItemStack;Lnet/minecraft/inventory/IInventory;)V", false));
				...
				while (nodes.hasNext()) {
					...
					if (node.getOpcode() == ASTORE) {
						...
						instructions.add(new MethodInsnNode(INVOKESTATIC, "com/jcoy0907/cccomponents/hooks/ComputerCraftHooks", "getName", "(Ljava/lang/String;Lnet/minecraft/item/ItemStack;)Ljava/lang/String;", false));
				...
				while (nodes.hasNext()) {
					...
					if (node.getOpcode() == ASTORE) {
						...
						instructions.add(new VarInsnNode(ALOAD, 1));
						instructions.add(new VarInsnNode(ALOAD, 7));
						instructions.add(new MethodInsnNode(INVOKESTATIC, "com/jcoy0907/cccomponents/hooks/ComputerCraftHooks", "update", "(Lnet/minecraft/entity/Entity;Lnet/minecraft/item/ItemStack;Ldan200/computercraft/shared/computer/core/ServerComputer;)V", false));
						m.instructions.insert(node, instructions);
						break;
					}
				}
