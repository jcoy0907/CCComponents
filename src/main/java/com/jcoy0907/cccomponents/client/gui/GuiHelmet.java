...
import com.jcoy0907.cccomponents.network.TextFieldInputEventPacket;
import com.jcoy0907.cccomponents.smarthelmet.AddButtonCommand;
import com.jcoy0907.cccomponents.smarthelmet.AddTextFieldCommand;
	...
	protected void keyTyped(char eventChar, int eventKey) {
		...
		for (GuiTextField text : textFields.values())
			if (text.textboxKeyTyped(eventChar, eventKey))
				CCComponents.NETWORK.sendToServer(new TextFieldInputEventPacket(UUID.fromString(Minecraft.getMinecraft().thePlayer.getCurrentArmor(3).getTagCompound().getString("identifier")), eventChar+"", text.getText(), "textboxEntry", Minecraft.getMinecraft().thePlayer.getDisplayName()));
